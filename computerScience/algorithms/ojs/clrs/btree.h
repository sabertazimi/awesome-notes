...
public:
     ...
     * 2 means character
     */
    void PrintTree(int kind = 1) const {
        queue<Node*> myqueue;
        myqueue.push(bRoot);
        int depth = 0;
        ...
        while (!myqueue.empty()) {
            ...
            while (!myqueue.empty()) {
                ...
                for (int i = 1;i <= temp->n;i++) {
                    if (kind == 2) {
                        cout << (char)temp->keyValue[i] << " ";
                    } else {
                        cout << temp->keyValue[i] << " ";
                    }
                }
    ...
    T get_minimum(void) const {
        return B_TREE_FIND_MIN(bRoot);
    }

    T get_maximum(void) const {
    ...
    bool remove(const T &key) {
        ...
        if (bRoot->n == 1 && bRoot->isLeaf) { // special case
            clear();
            return true;
        }
...
private:
    ...
    void recursive_remove(Node *pNode, const T &key) {
        ...
        if (i <= pNode->n && key == pNode->keyValue[i]) {
            if (pNode->isLeaf) {
                //从pNode中删除k
                pNode->n--;
                ...
                if (pChildPrev->n >= t) {
                    ...
                    return;
                } else if (pChildNext->n >= t) {
                    //节点pChildNext中至少包含CHILD_MIN个关键字
            ...
            if (pChildNode->n == t-1) {
                ...
                if (pLeft && pLeft->n >= t) {
                    ...
                    --pLeft->n;
                } else if (pRight && pRight->n >= t) {
                    //右兄弟节点至少有CHILD_MIN个关键字
