...
- 四对子: 进张较差时, 可考虑七对子役种.
- **早巡**不应固定雀头, 应该把雀头拆开和其他牌组成搭子, 中晚巡自然进张形成雀头.

### 搭子牌效

- 搭子: 两面 > 连嵌 > 中张嵌张 >> `24`/`68`嵌 > `13`/`79`嵌 > 边张.
- **五块理论**: 五搭子饱和, 六搭子不利.
- 五搭子形基本上只要切掉价值最低的浮牌.
- 有效牌重复效应:
...
- 二度进张**恶型**:
  ...
  - `6889`: 边张二度进张, 超级差, 当搭子超载时, 优先切掉 `9`.
  - `3467`: 嵌张二度进张.
  - `3344`: 完全二度进张, 只有作一杯口或双碰听时有价值.
- 强度: 四连型 > 中膨型 >> 亚两面型 > 跳一型 > 二度进张恶型.
