...
- 门清时 (立直或平和):
  - 考虑切掉一组对子 (保证向听数不减少): 切两面对 -> 切嵌张对 -> 切边张对.
  - 考虑**向内靠拢**, 切幺九对.
...
- 搭子超载时, 优先切掉弱搭子:
  ...
  - 弱搭子: 嵌张搭, 边张搭, 损失手役牌 (幺九牌), 绝张搭.
  - 优先切掉最弱搭子, 暂时保留中张浮张, 后续形成中张好型搭子后, 继续切掉最弱搭子, 直至剩下至多五组搭子.
...
- `5679`: `9` **不再**受制于重复牌效应, 进 `8` 可以形成五连型好型.
...
- 跳对子型: `335577`, **跳对子型切中间对子**, 一般选择切 `5`, 固定中间连嵌好型.
...
- 拆暗刻.
- 七对子: 最强防守牌型, 又有和牌可能.
- 中巡好型搭子足够时, 可考虑保留安全牌进行防守, 亦可用安全牌追立.
