...
- 嵌张一杯口型: `33455` -> `345` + `35`
  ...
  - 存在靠张时, 形成隐藏的两嵌型后, 可转化成**两面两嵌型**.
- 两面中嵌型: `23578` 切 `5` (有效牌重复).
- 两嵌预备型:
  - `24579` 进张 `3` -> `234579`, `68` 两嵌进张.
...
- 三面听:
  ...
  - 多面听时, 三暗刻也可看作`雀头`+`单张`.
- 不规则多面听:
  - 基本策略: 暗刻型既可看成`面子`, 又可看成`对子+单张`, 从而形成不规则多面听牌.
  ...
  - 暗刻四连型: `2224567` 听 `347`, 嵌张 + 单骑.
  - 暗刻跳一型: `4445679` 听 `89`, 嵌张 + 单骑.
  - 暗刻单骑型: `3335777` 听 `456`, 嵌张 + 单骑.
...
- **两张以上宝牌**时候要重视速度与和牌的容易度,
  把鸣牌作为做牌的前提.
- 高打点形状极差手牌: 积极寻找手役, 副露鸣牌加速和牌.
- 低打点快速副露和牌: 阻止对手高打点和牌, 亲家保持连庄, 子家快速过庄.
- 可以考虑鸣牌:
  - 满贯手牌: e.g 多宝牌/染手.
...
- 幺九牌容易鸣牌, 中张数牌不容易鸣牌.
- 多副露对对和 Nomi 收益低,
  需复合其他手役 (Dora/役牌/混一色/全带幺/三暗刻/混老头) 作满贯.
- 可以考虑对对和手役 (七对子转对对和):
