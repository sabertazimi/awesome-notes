...
- 搭子超载时, 优先切掉弱搭子:
  ...
  - 搭子超载全为好型时, 优先**切掉中张浮张** (降低手牌危险度), 然后切掉超载搭子.
...
- 可以考虑鸣牌:
  ...
  - 两副露基准: **3900** 点, 良型一向/两向听 (防止过度鸣牌).
  - 三副露基准: **7700** 点, 良型听牌/一向听 (防止过度鸣牌).
  - 晚巡低向听 (急于听牌).
  - 破除一发.
  - 速攻阻止对手大牌.
- 不建议鸣牌:
  - (早巡) 牌型好, 打点低: 全好型手牌可以作门清手役, 提高打点.
...
- 当配牌同色牌多/幺九搭子多/对子多/愚型多/牌型极散时:
  **不应按牌效直线作牌**, 应采取**防守反击**策略,
  ...
  手牌多为幺九边张, 弃和时拥有足够安全牌.
- 领先时, 速攻 (役牌副露/断幺九副露/默听) 抢庄是保持一位的有效手段.
...
- 立直和默听打点界限为**满贯**.
- 满贯以上, 东一局或点数不足 (跳满/倍满/四位) 时可以考虑立直, 其余情况一般选择默听.
- AL + TOP + 确定手役, 选择默听.
- **低打点听牌**时, 默听可以有**弃和**机会:
- **愚型听牌**时, 默听可以有**改良**机会:
  - 子家早巡愚型听牌, 当改良枚数是和牌枚数两倍以上时, 可以默听寻求改良.
  - 亲家早巡愚型听牌, 可以考虑立即立直.
  - 早巡 + `1` ~ `2` 番牌 (e.g **立直 Nomi**): 可以考虑拒听, 切愚型搭子提高打点 (e.g 切客风对子形成断幺九手役, 切边张搭子形成平和手役).
- **愚型默听**和牌率 ~= **良型立直**和牌率.
...
- 良型听牌应立即立直.
- 先制立直时, 愚型听牌高于良型听牌两番时, 可以损失一定和率, 追求高打点.
- 追立时, 应优先考虑和率 (对攻成功率), 一定程度上放弃高打点.
- **平场**或**亲家**时, 低打点良型听牌立直 > 高打点愚型听牌立直 > 默听.
- **非平场**或**子家**时, 立直 `Nomi` 时需要考虑立直风险, 不可激进对攻.
- 四暗刻:
  - 多面听立直率大于 50%: 立直可以提高打点, 立直逼迫对手防守, 延长巡目提高自摸和率.
  - 一面听立直率小于 1%: 役满确定, 无需提高打点, 存在换听可能.
...
- Dora 多 + 牌型极散, 保留单张役牌, 等待役牌对子, 寻求快速听牌.
...
- 处理浮牌与复合型时, 时刻考虑**宝牌进张**的影响.
  ...
  - 宝牌对子: 保留 2 组对子, 保留 Dora 搭子形成 Dora 刻子可能性 (2 番 -> 3 番).
...
- 七对子和率: 七对子进张数少, 听牌数少, 需要注重牌河作成 (引卦), 提高和率.
  - 早外引卦: `79` / `89` 前三巡早切 `7` / `8`, 提高 `9` 和率.
  - 壁筋引卦: `58` / `69` 前三巡早切 `5` / `6`, 提高 `8` / `9` 和率.
  - 单骑引卦: 听熟张/绝张字牌, 提高和率.
  - 在手牌条件允许下, 早巡按照字牌-幺九-中张顺序切若干张牌, 隐藏七对子手役, 提高和率.
- 七对子**对攻能力较弱**:
  - 追立需谨慎, 可考虑**兜牌或弃和**.
...
- 手切一组**愚型搭子** (e.g `75` 嵌张搭): 愚型搭子超载, 手牌**速度较快**.
- 手切一组**好型搭子** (e.g `56` 两面搭): 好型搭子超载, 手牌**速度较快**, 好型听牌概率大, 可用**筋壁理论**防守.
- 手切**幺九对子**: 作断幺九手役, 手牌**速度较快**.
- 手切**唯一役牌对子**: 作断幺九/清一色手役, 手牌**速度较快**, **打点较高** (存在立断平可能).
...
- 逆序切牌立直: 高概率非常规手役 (对子手役), 壁筋理论失效.
- 立直若干巡后, 其他家未切的**立直家现物**高概率**牌山确**.
- 立直若干巡后, 未通过筋牌的危险度: 牌河中枚数越少的筋牌 (e.g 0 或 1 枚), 危险度越高.
- 立直若干巡后, 未通过筋牌的危险度: 手牌中枚数越多的筋牌 (e.g 暗刻 `22555p`), 危险度越高.
...
- 放铳危险度:
  ...
  - 对手打点: 亲家/子家, 牌河 Dora 出现枚数, 副露打点.
  - 巡目越晚, **半筋/无筋牌**放铳概率越高.
  - 局数越晚, **放铳吃四**概率越高.
  - 两家对攻, 放铳概率高: e.g 子家立直, 亲家对日时, 手牌打点低或形状差时应完全弃和.
- 弃和成功率.

> 基本对攻策略 (对手立直或三副露): 听牌攻, 一向听攻一巡, 高向听弃和.

**打点**是对日的准入资格, **形状**是对日的胜率保障, **放铳危险度**是对日与否的参考依据.
