...
- 当配牌同色多/幺九搭子多/极差时, 可以考虑全带/混一色/七对子/国士无双等幺九手役:
  手牌多为幺九边张, 弃和时拥有足够安全牌.
...
- 早巡 3 对子 + 0/1 面子 + 多愚型, 可以考虑七对子手役:
  ...
  - 七对子手役手牌灵活, 可以提前留安, 保证弃和时**拥有足够安全牌**.
...
- **摸切**: 存在周边牌可能性下降.
- **手切**: 存在周边牌可能性上升 (`-2/+1` 两面听与 `-1/+2` 两面听).
- 早巡多次摸切: 超好型或进张极差.
...
- 手切一组**愚型搭子** (e.g `75` 嵌张搭): 愚型搭子超载, 手牌速度较快.
- 手切一组**好型搭子** (e.g `56` 两面搭): 好型搭子超载, 手牌速度较快, 好型听牌概率大, 可用**筋壁理论**防守.
- 手切**唯一役牌对子**: 作断幺九/清一色手役, 手牌速度较快.
- 手切三枚同数牌 (e.g 6m6p6s): MPS 法则, 手牌速度较快.
...
- 切宝牌存在两种可能：
  - 满贯以上高打点 (放弃宝牌).
  - 好型一向听/听牌 (孤张宝牌).
- 手切宝牌周边搭子 (e.g Dora `3p` 手切 `12p`): 宝牌对子/刻子.
- 通过**牌河宝牌**个数推测对手手牌打点.
...
- 立直若干巡后, 其他家未切的立直家现物**在牌山概率上升**.

#### 读牌实例

`9p1z5z4p` 长考立直: 大概率 `68p` 听 `7p`.
