...
- 相同进张: 搭子直接模切, 成型面子**空切**:
  - Slide: 使对手误以为成型面子靠张危险.
  - Reset: 重置对手安全牌判断.
- 尽早切掉连筋孤张:
  - 有效牌重复效应.
  - 中晚巡切牌时容易放铳.
- **早巡对手捨牌**的**靠张**进张较为容易, 称为**良好场况**:
  - 进张可能: 靠张高概率**牌山确**.
...
- 不建议鸣牌:
  ...
  - 鸣牌**过早**或副露**过多**都会导致他家提高警惕性, 使得**和牌率降低**:
    - e.g 若 `68s` 先于 `w东对子` 鸣牌, 则 `w东对子` 鸣牌难度大幅度提升.
...
- 当配牌幺九搭子多/同色牌多/对子多/愚型多时:
  **不应按牌效直线作牌**, 应采取**防守反击**策略,
  可以考虑全带/混一色/七对子/国士无双等幺九手役,
  手牌多为幺九边张, 弃和时拥有足够安全牌.
...
- 亲家作七对子优先考虑和牌率 (保留**幺九对子**), 子家作七对子可考虑打点 (断幺九 + 七对子).
...
- **摸切**: 存在靠张可能性下降.
- **手切**: 存在靠张可能性上升 (`-2/+1` 两面听与 `-1/+2` 两面听).
- 早巡多次摸切: 超好型或进张极差.
- 早巡切牌多为**断幺牌**, 后切**生张字牌**:
  高概率非常规手役 (全带/染手/对子/国士无双), 壁筋理论失效, 手牌速度不确定 (一般慢于正常顺子手役).
