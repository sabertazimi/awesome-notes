...
- 搭子强度: 两面 > 连坎 > 中张坎张 >> `24`/`68`坎 > `13`/`79`坎 > 边张.
- 双碰进张优于连坎进张, 特别是在有役牌时, **碰**的几率远大于**吃**.
...
- 边张对: `122`.
- 连坎张: `246`.
  - 两面坎张: `2` + `456` + `67` -> `246` + `567`.
  - 长连坎张: `134568`, 进 `2` 或 `7` 都可形成两面子.
...
- 跳对子型: `335577`, **跳对子型切中间对子**, 一般选择切 `5`, 固定中间**连坎好型**, 保留**边张雀头**可能,
...
- 接近听牌时 (不再缺少搭子), 可考虑切掉挤占手牌空间的四牌复合型固定面子, 保留三牌复合型 (两面对/坎张对/连坎张).
