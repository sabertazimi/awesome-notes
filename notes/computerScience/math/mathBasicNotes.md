...
- `2` `8`.
- `3` `4` `5` `6` `7`.
- 搭子进相同张直接模切, 成型面子进行空切 (使对手误以为成型面子周围危险).
- 早巡对手捨牌的**周边牌进张较为容易**:
...
- 满贯/跳满以上, 一般选择默听.
- AL + TOP + 确定手役, 选择默听.
- **低打点 + 愚型听牌**时, 默听可以有**改良与弃和**机会: e.g 一番愚型听牌断幺.
- **愚型默听**和牌率 ~= **良型立直**和牌率.
...
- 边张 > 生张 > 宝牌生张.
- 染手没有筋牌防守理论, 同色牌都是**危险张**.
- 对对和没有筋牌防守理论, NO CHANCE, 役牌, 生张都是**超危险张**.
- 鸣牌后切出的同色牌危险度提升.
...
- 切掉一组搭子 (e.g 连续手切 `75`), 说明搭子超载, 手牌进度较快.
- 切掉一组好型搭 (e.g `56` 两面搭), 说明好型搭超载,
  听两面好型概率很大, 可以利用筋壁理论进行防守.
- 切宝牌存在两种可能：
  - 满贯以上高打点 (放弃宝牌).
  - 好型一/二向听 (孤张宝牌).
- 早巡多次摸切: 超好型或进张极差.
