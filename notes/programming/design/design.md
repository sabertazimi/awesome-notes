---
sidebar_position: 1
tags: [Programming, DevOps, Design Pattern]
---

# Design

## Architecture

- [Architecture](./architecture.md) - Software architecture and design principles

## Paradigms

- [Paradigms](./paradigms.md) - Programming paradigms and frameworks

## Design Patterns

- [Patterns](./patterns/patterns.md) - Design patterns catalog

## Principles

- [SOLID](./solid.md) - SOLID principles

## Practices

- [Refactoring](./refactoring.md) - Code refactoring techniques
- [Code Review](./review.md) - Code review guidelines
