...
- `<div><img src="1.jpg"></div>` 下边缘存在空隙:
  `Strut` 默认基于[基线对齐](./inline.md#baseline).
...
`%` margin calculate by `containing block` inline size (`width`),
...
`%` padding calculate by `containing block` inline size (`width`),
