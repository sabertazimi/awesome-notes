...
- 愚形复合形包含**唯一雀头**时, 可以考虑固定雀头 (e.g. `112`).
- 役牌为**唯一雀头**时, 若牌形较好(一向听), 且立直门清打点显著高于役牌打点, 则不急于鸣役牌.
- 客风对子:
  - 攻可作雀头听平和, 守可作安牌防一发.
...
- 两向听时:
  - **一向听顶峰理论**: 最大化**一向听进张**数, 在两向听时可考虑保留三组对子, 切掉较弱三牌复合形.
- 门清时 (立直或平和):
...
- 四连形, 中膨形, 亚两面形混合时, **切掉亚两面形**, 高概率是正解.
- 多种复合形混合成复杂形状时, **保留复杂形状, 切牌固定两面搭子或完整面子**, 高概率是正解.
- 早巡重视改良数与速度, 中巡重视进张数与安全度.
- 中巡好形搭子足够时, 可考虑**保留安全牌**进行防守, 亦可用安全牌进行追立:
...
- 五搭子形基本上只要切掉价值最低的浮牌.
- 打点足够时重视好形搭子, 保留浮牌寻求**好形改良**, 最大化进张数.
- 打点不足时重视手役搭子, 保留浮牌寻求**打点改良**, 提升手牌打点.
- 六搭子 (6 Block):
  保留用于形状改良 (存在愚形搭子) 与打点改良 (断平三色可能)
  的孤张浮牌或超载搭子,
  防止手牌形成低打点愚形听牌.
- **低效率二度进张**:
...
- 全好形时, 优先**切掉中张浮张** (降低手牌危险度), 然后切掉超载搭子.
- 存在愚形时, 优先切掉最弱搭子, **保留中张浮张**, 后续形成好形搭子后, 继续切掉最弱搭子, 直至剩下至多五组搭子.
- 存在愚形与**两对子**时, 可以切掉愚形搭子, 保留中张对子形成好形搭子后, 另一组对子可作雀头.
- 存在复合形时, 可以先切边张: 价值低.
- 不存在复合形时, 可以先切中张: 危险度高.
...
- 边张对: `122`.
- 两坎形: `246`.
  - 两面两坎形: `2` + `456` + `67` -> `246` + `567`.
  - 长两坎形: `134568`, 进 `2` 或 `7` 都可形成两面子.
  - 弱于两面搭子: 需要 3 张牌, 听牌形为坎张听.
- 坎张对利于副露鸣牌, 两坎形利于门清平和.

搭子进张成为搭子 + 对子复合形, 称该进张为**补强牌**:
...
- 三对子不利:
  当出现三对子 (特别是**门清一向听**) 时,
  切掉**搭子部位较强**的复合形的**对子** (e.g. 两面对),
  **让补强牌辅助较弱的搭子**,
  固定强搭子牌形 (两面对 - 对子补强牌 = 两面搭),
  即**固定好形**.
  其中, 四连形与亚两面形都可以看作雀头候补形.
- 副露作牌时, 可以适当保留三对子, 碰听加速胡牌:
  - 愚形较多.
  - 双东对子.
  - 宝牌对子.
...
先根据五块理论与中膨复合形, 将手牌分为 `133m` + `456m` + `67m` + `123s` + `788s`.
可以此时已经形成了有效的五块搭子, 需要切掉两个三牌复合形中的一个, 保持张数的正收益.
对于`对子` + `搭子`的复合形, 可以采取固定好形的策略, 切掉 `8s`, 固定 `78s` 两面形.
...
- 好形: `3456` -> `34` + `56`, 常常将其视为**两面 + 两面**, 而不是单骑进张.
- 次好形: `1234` -> `123` + `4`, 常常将其视为单骑进张, 而不是**边张 + 两面**.
- `3456` 与 `4567` 8 种 28 枚进张, 进张数最多的四牌复合形.
- 缺少搭子或**存在愚形**时, 四连形具有良好**延展性**, 容易形成新的**两面搭**, `4567` -> `34567` / `45667`.
- 缺少对子时, 四连形具有良好**延展性**, 容易形成新的**雀头**, `4567` -> `44 567`.

#### 中膨形

- 进张: 五种进张, `3445` 存在 `23456` 进张.
- 好形: `3445` -> `34` + `45`, 两面 + 一杯口, **好形进张 + 一杯口进张**.
- 缺少搭子或**存在愚形**时, 中膨具有良好**延展性**, 容易形成新的**两面搭**, `4556` -> `34556` / `45566`.
- **恶形**: `1223` -> `123` + `2`, 价值较低, 接近于单张 `2`, 不存在连续形时, 可优先切掉 `2`.

#### 亚两面形

- `3345` -> `34` + `35` / `33` + `45` / `3` + `345`.
- 缺少对子时, `3345` 有一定价值, 稍稍高于单张 `3`.
- 不缺对子时, `3345` 价值较低, 接近于单张 `3`, 可优先切掉 `3`, 保留其他复合形.
- **恶形**: `1123` 价值接近于单张 `1`, `7899` 价值接近于单张 `9`.
- 总体来说, 亚两面形作两面能力**与孤张无太大区别**.
- `3345` -> `34` + `35`, 可以通过鸣牌改良成 `34` 两面.
- 亚两面形可以直接用于听牌, 存在转二杯口可能性, 可以形成 `33345` 暗刻复合形.

| 复合形          | 靠张 | 两面 | 坎张 | 双碰 (雀头) |
| --------------- | ---- | ---- | ---- | ----------- |
| 四连形 `3456`   | 1~8  | 2457 | 18   | 36          |
| 中膨形 `3445`   | 2~6  | 2356 |      | 4           |
| 亚两面形 `3345` | 1~6  | 24   | 15   | 36          |

| 复合形   | 进张数 | 好形率 | 雀头固定形 | 雀头流动形 |
| -------- | ------ | ------ | ---------- | ---------- |
| 四连形   | S      | A      | A          | S          |
| 中膨形   | B      | S      | S          | B          |
| 亚两面形 | A      | B      | B          | A          |
...
- `5679`: `9` **不再**受制于重复牌效应, 进 `8` 可以形成五连形好形, 形成 `56` 两面搭.
- `3457` 可以形成好形 `34567`, 利用**吃**可以形成两面子.
- 靠张一向听时, **暗刻跳一形**牌效差 (e.g. `1113s` 切 `3s`, 保留其他两张浮牌).
- 各种形状的跳一形 (三连跳一形, 两面对跳一形, 四连跳一形, 中膨跳一形, 亚两面跳一形, 暗刻跳一形, 五连跳一形)
  在搭子不足时可以形成新的好形搭子, 当搭子足够时牌效较差, 切掉跳一**浮张**进张损失较小.
...
- `3334` -> `33` + `34`, 容易形成`面子` + `雀头`:
  ...
  - 可以形成平和形, 提升手牌打点.
- **一向听**同时存在**两种四牌复合形**时, **复合形切牌顺序**:
  - 跳一形: 牌效最差.
  - 亚两面形.
  - 中膨形.
  - 四连形: 保留三面听牌可能.
  - 暗刻复合形: 保留平和可能, 保留三面听牌可能.

#### 二度进张复合形

- `3556` -> `35` + `56`: 存在二度进张 `4`, 当搭子超载时, 优先切掉 `3`.
- `3467`: 坎张二度进张.
- `3344`: 完全二度进张, 只有存在其他复合形 (一杯口) 或双碰听时有价值.
- **恶形**`6889`: 边张二度进张, `69` 连筋恶形, 当搭子超载时, 优先切掉 `9`.

#### 四牌复合形强度

- 四牌复合形**强度**: 四连形 > 中膨形 >> 亚两面形 > 跳一形 > 二度进张恶形.

`1234p` 与 `1123s`:

当手牌缺少对子时, 四连形中的 `1p` 可以解放 `4p`, 进张下一个 `4p` 时, 可以形成面子 + 雀头,
即 `1p` + `234p` -> `123p` + `4p`, 使得 `1p` 价值约等于 `4p`.

亚两面形中的 `1s` + `123s` 无法解放 `3s`, 使得 `1s` 价值约等于 `1s`.
比较 `4p` 与 `1s` 价值, 可以得出此时应该切 `1s`, 保留 `1p`.

### 多牌复合形

- 五连形: 进张超好形, 且容易形成雀头.
- 五连复合形: `234556`, 进张 `1457`.
- 暗刻复合形:
  - 暗刻可作面子, 可作对子, 扩大进张面.
  - `333` + `45` -> `33` + `345`, 容易形成`面子` + `雀头`, 缺少对子时, 价值上升.
  - `344457`: 两坎暗刻形, 进张 `46`.
  - `355567`: 坎张对暗刻形, 进张 `458`.
  - `344456`/`345556`: 两面对暗刻形, 进张 `2457`.
  - `334445`/`344455`: 中膨暗刻形, 进张 `2345`/`3456`.
  - `334555`/`333455`: 中凹暗刻形, 进张 `2345`/`3456`.
  - `344555`/`333445`:
    阶梯暗刻形, 进张 `23456`,
    同时横向与纵向延展, 兼顾平和/一杯口/对对和, 通常优于四连形.
  - `2345666`: 进张 `25`/`147`.
- 暗刻跳一形: `333` + `5`, **暗刻**旁边的**浮牌不适合靠张一向听**.
- 两顺子形:
  - `344556` -> `345` + `456`.
...
- 坎张一杯口形: `33455` -> `345` + `35`
  ...
  - 存在靠张时, 形成隐藏的两坎形后, 可转化成**两面两坎形**.
- 两面中坎形: `23578` 切 `5` (有效牌重复).
- 两坎预备形:
...
- **两面两坎**预备形:
  ...
  - `3356778` 切 `3`.
  - `3556778` 切 `5`.
- **两面两坎形**: `455679` -> `456` + `579`, 进张 `368`.
  - 当追求进张数时, 保留 `455679` 形成 `456` + `579`.
  - 当**缺少雀头**或追求**好形率**时, `455679` 切 `9` 形成 `45` + `567`, 损失进张 `8`.
- 宝牌连续形: `11234` (`4` 为宝牌), 用筋牌 `1` 作雀头, 摸入 `4` 时可切 `1` 极具迷惑性.
- 跳对子形: `335577`, **跳对子形切中间对子**, 一般选择切 `5`, 固定中间**两坎好形**, 保留**边张雀头**可能,
  `33577` 进张能力很强, 是一组强力复合形.
- 双雀头天平形:
...
- 八枚两翼对称形 (两组中膨形):
  `34456778` -> 顺子 + 两坎形 + 对子 (浮动雀头 `44` 与 `77`),
  可以保留此复合形, 也可以切 `8` 固定雀头与断幺.
- 九枚两翼对称形 (两组中膨形 + 中张):
  `233456778` -> 两组顺子 + 两坎形, 进张 `1469`, 保留此复合形.
...
- `面子` + `雀头` + 3 `搭子` + 孤张 (**五搭子**):
  - 全好形时, 优先切掉最弱孤张.
  - 存在愚形时, 可以切掉最弱搭子, 保留孤张改良可能.
- `面子` + 3 `对子`搭 + 孤张 (**四搭子**):
  - 愚形较少时, 可以切掉两面对补强牌, 固定两面好形, 保留孤张改良可能.
  - 愚形过多时, 重视七对子/对对和进张.
- `面子` + `雀头` + 4 `搭子` (**六搭子**):
  - 全好形时, 可以切掉两面对补强牌, 固定两面好形, 保留六搭子形.
  - 存在愚形时, 优先切掉最弱搭子.
- 2 `面子` + **无雀头**两向听:
  - 全好形时, 重视雀头进张, 重视易形成雀头的复合形 (如四连形与亚两面形).
  - 存在愚形时, 重视改良进张, 保留孤张改良可能,
- 一向听顶峰理论:
  全好形低向听时, 可考虑切掉多余四牌复合形 (固定面子),
  保留三牌复合形, 最大化一向听到听牌的进张数.
- 存在愚形搭子时 (容易形成愚形听牌), 可以考虑暂时保留四牌复合形 (不急于固定面子), 寻求**好形搭子改良**.
- 多牌复合形的价值在缺少搭子的情况下价值较高 (进张面广, 进张数多), 在不缺搭子的情况下只能用于改良牌形与打点.
- 两向听时可以保留三对子形, 进入一向听后再进入两对子形 (三对子不利理论).

### 一向听策略

- 一向听类别: 愚形一向听, 完全一向听, 靠张一向听, 无头一向听.
- 全牌效策略.
- 听牌形重视策略 (愚形轻视策略).
- 和牌率重视策略 (愚形轻视策略).
- 速度重视策略:
...
- 宝牌靠张.
- 断幺九/平和: 切**边张**搭子/对子, 追求断幺九或平和好形.
- 染手: 切同色搭子 (甚至面子).
- 三色同顺.
- 多愚形高向听手牌, 早巡退向寻求形状与打点改良, 保留立直 Nomi 变成立断平三色的可能.
- 早巡低打点愚形一向听应退回两向听寻求形状与打点改良:
  - 早巡愚形 Nomi 确定的一向听 (特别是子家), 应退向寻求形状与打点改良,
    e.g. 4巡目西家 `266m123789p11355s` 切 `6m` 退向.
  - 保留四连形与中膨形, 切掉愚形搭子, 退回两向听, 寻求形状改良.
  - 保留手役靠张, 切掉愚形搭子, 退回两向听, 寻求打点改良.
- 存在复合形的低向听手牌, 早巡退向可以带来至少 2 番打点改良时, 应选择退向甚至拒听:
  e.g. 立直 Nomi 边坎听牌 -> 立断平好形听牌.
...
- 包含愚形搭子的一向听, 需要积极寻求改良
  - 固定好形搭子, 保留中张数牌进行靠张.
  - 早巡拆愚形搭子退回两向听, 寻求打点和形状.
- [当两面搭子与雀头候补形成复合形时](https://www.bilibili.com/video/BV1X14y1R7bS),
  基本上要切孤张数牌, 保留复合形:
  e.g. `1378m123p6p334789s`, 切 `6p`,
  进 `1m`/`3m` 形成完全一向听, 进 `2s`/`5s` 形成无头一向听.
- 巡目尚早或追求改良时, 可以考虑 (需慎重) 拆愚形搭子进行退向, 留下高价值浮牌 (e.g. 中张宝牌).
...
- 当一组搭子为愚形搭子, 暗刻为复合形时:
  拆掉愚形搭子, 追求高打点和好形听牌,
  e.g. `6667m34p33455s789s` 切 `3s`.

无头非暗刻一向听:

- 当两组搭子形成复合形时:
  进张数多于完全一向听, 好形率接近, 应选择无头一向听 (固定三面子),
  e.g. `789m123p2356778s`, 进张 `14s` 时, 形成多面好形听牌, 而不是普通的单骑愚形听牌.
- 当两组搭子均为两面搭子时:
  - 进张数多于完全一向听 (28 > 20), 但好形率低, 应选择完全一向听 (固定雀头).
  - 中晚巡或两面损失 (枚数少/二度受) 时应重视进张枚数, 应选择无头一向听 (固定三面子).
- 当一组搭子为愚形搭子时,
  进张数多于完全一向听 (24 > 20), 好形率接近, 应选择无头一向听 (固定三面子).
- 当重视其中一个搭子进张时, 可以切掉另一组搭子:
  e.g. `34s` + `24m` (`2s` 为宝牌) 无头一向听,
  `34s` **好形**搭子进张均为**宝牌**,
  可以切 `2m` 固定 `34s` 搭子.
- **单骑听牌**时, 损失平和打点且和牌率低, 可以考虑默听改良或直接拒听.
...
- 靠张一向听进张数一般为 30+~40+ 枚:
  - 优先考虑听牌形状和手牌打点, 选择**好形听牌**/**高打点**浮牌靠张,
    总枚数不是第一考虑因素 (靠张一向听进张很广).
  - 两张浮牌与面子/雀头形成复合形后, 进张数进一步增加.
- 进张数大于完全一向听, 可能形成**愚形听牌**, 优先考虑**听牌好形率**进行切牌.
- 中张四连形 (进张/雀头) > 中张中膨形 (好形) > 中张亚两面形 (雀头) >
  `37` > `46` > `5` >
  边张四连形 > 边张中膨形 > 边张亚两面形 >
  `28` > `19`.
...
- 早巡/不含复合形/需要打点时, 固定雀头形成两面两面一向听, 提升手牌打点.
- 中晚巡/包含复合形时, 固定面子形成无雀头一向听, 最大化进张数.
...
- 单骑听:
  ...
  - 存在暗刻时, 单骑向复合形靠近, 保留换听两面可能:
    e.g. 单骑听中膨形中间牌, 进张后暗刻改雀头换听两面.
...
- 不规则多面听:
  - 基本策略:
    ...
    - 暗刻复合性: 暗刻形既可作`面子`, 又可作`对子+单张`, 形成多面听牌.
  - 一杯口单骑形: `2233344` 听 `234`, 双碰 + 单骑.
  - 一杯口双碰形: `33445566m99p` 听 `369`, 双碰.
  - 暗刻双碰形: `44456m99p` 听 `479`, 两面 + 双碰.
  - 暗刻亚两面形: `1112234` 听 `235`, 两面 + 边张.
  - 暗刻中膨形: `5667888` 听 `467`, 两面 + 单骑.
  - 暗刻四连形: `2224567` 听 `347`, 坎张 + 单骑.
  - 暗刻跳一形: `4445679` 听 `89`, 坎张 + 单骑.
  - 暗刻单骑形: `3335777` 听 `456`, 坎张 + 单骑.
...
- 骗筋: 听牌河 (现物/立直宣言牌) 筋牌.
- 偷现: 听`立直家`/`副露家`/`亲家`现物.
- 当听牌枚数过少 (e.g. 2 枚) 或听牌场况过差 (e.g. 愚形听牌且牌河同色牌过少) 时, 可以选择**拒听**寻求改良:
  - 无头一向听 (3 面子 + 2 搭子) 进张形成中张雀头 (`344`), 听牌形为边张听时 (`12`),
    可以选择切掉雀头 (`4`) 拒听, 寻求改良 `12` 边张搭子.

## 鸣牌策略

- 鸣牌是赤牌麻将中非常重要的技术.
- 鸣牌要点: 鸣牌形状 (役牌/愚形), 手牌形状, 手牌打点.
- 鸣牌优点: **速度上升**.
- 鸣牌的缺点: **打点下降**, **防御力下降**.
- 可副露时三牌/四牌复合形价值上升: 鸣牌加速和牌, 食延消解愚形.
...
- 可以考虑鸣牌:
  - 满贯手牌: e.g. 多宝牌/染手.
  - 消除愚形 (坎张吃, 边张吃, 绝张吃/碰): 愚形过多, **门清立直速度过慢**.
    - e.g. `46` 吃 `5`, 消除愚形搭子.
    - e.g. `7789` 吃 `8`, 留下 `78` 两面好形搭子, 切掉另一组愚形搭子.
    - e.g. `4678` 吃 `5`, 留下 `78` 两面好形搭子, 切掉另一组愚形搭子.
  - 断幺九副露基准: **3900** 点, 序盘从**愚形**开始鸣牌.
  - 两副露基准: **5200** 点, 好形一向/两向听 (防止过度鸣牌).
  - 三副露基准: **7700** 点, 好形听牌/一向听 (防止过度鸣牌).
  - 晚巡低向听 (急于听牌).
  - 破除一发.
  - 速攻阻止对手和牌.
- 不建议鸣牌:
  - 远而小副露: 牌形差, 打点低, 鸣牌加速收益较低, 不鸣牌可保持高打点和防御力.
  - 损打点副露:
    - **早巡**牌形好, 打点低: 全好形手牌可以作门清手役, 提升打点.
    - 鸣牌降低一半以上打点.
...
- 鸣牌迷彩:
  - 能愚形吃的情况下, 优先选择愚形吃: 好形吃会提升别家的警惕性.
  - 能保留安全牌的情况下, 优先选择保留安全牌:
    e.g. `23345m` 用 `35m` 吃 `4m` 保留 `2m` 安全牌.
...
- 不建议杠牌:
  - 牌形差, 多愚形, 高向听.
  - 进张减少 (甚至退向): e.g. 中张暗刻不开杠, 保留作搭子 (暗刻复合形).
  - 他人立直, 自己和牌率较低.
  - 手牌打点足够, 无需杠宝牌.
...
- 打点 (番数) 相同时, 确定的手役比不确定的手役更有利:
  - 断幺九接近成形, 可以选择切掉役牌对子 (未成形), 保留其他中张的对子或搭子, 选择断幺九手役.
  - 断幺九并未成形, 可以选择保留役牌对子, 切掉其他超载的对子或搭子, 选择役牌手役.
...
- 愚形立直与默听基本准则:
  ...
  - 巡目: **8** 巡后默听.
- AL + TOP + 有役, 选择默听**局消化**.
- **愚形**听牌时, 默听有**改良**与**弃和**机会:
  - 亲家**早巡愚形**听牌, 可以先制立直.
  - 子家**早巡愚形**听牌, 当改良枚数是和牌枚数两倍以上时, 可以默听寻求改良.
- **早巡** `1` ~ `2` 番牌:
  - 可以考虑拒听, 切愚形搭子提升打点:
    - e.g. **早巡**手牌含复合形 + 平和可能, 切愚形搭子拒听, 退回好形一向听, 作平和手役.
    - e.g. **早巡**手牌含复合形 + 断幺可能, 切幺九搭子拒听, 退回好形一向听, 作断幺手役.
  - 平和/断幺 NOMI:
    - 一般情况下可以先制立直.
...
- **两面**立直, **字牌双碰**立直, **NC 壁外数牌双碰**立直, 均为**好形听牌**.
- 两面立直 > 振听三面立直 > 愚形立直 = 振听两面立直.
- **早中巡好形**听牌应先制立直.
- **无役多宝牌愚形**听牌应先制立直.
- **平场**或**亲家**时, 低打点好形听牌立直 > 高打点愚形听牌立直 > 默听.
...
- 早中巡**二向听**手牌, 通常是**鸣牌断幺九** (鸣牌后切幺九牌) 的好开始点:
  - **一向听**手牌可以追求门清.
  - **高向听无宝牌**恶形手牌不急于鸣牌: 鸣牌对于高向听帮助不大, 反而降低打点与防御力.
- **多张宝牌**: 和牌速度至关重要, **鸣牌断幺九**可以加速和牌.
- **亲家**愚形搭子过多时, **愚形搭吃牌** + **断幺九**可以加速和牌.
...
- 三枚役牌进张数约等于两坎形.
- 好形搭子超载时, 优先切掉单张役牌.
- 愚形搭子超载时, 可以保留单张役牌, 等待役牌对子, 寻求快速和牌.
- 宝牌多 + 牌形极散, 保留单张役牌, 等待役牌对子, 寻求快速和牌.
- 切役牌对子: 牺牲速度, 提升打点. e.g. 役牌 Nomi -> 立断平一杯口
- 当打点提升不大时, 保留役牌对子, 寻求快速和牌.
- 手牌有役牌对子或刻子 (特别是两番役牌对子) 时,
  可以保留其他孤张字牌, 保留混一色或对对和满贯可能.
- 后付役牌:
  - 可以优先消除愚形鸣牌, 后付役牌鸣牌难度不大, 且可以作安全牌.
  - 对子数量越多, 后付越安定: e.g. 只有役牌对子 (唯一雀头) 时, 比较难以进行后付.
- 役牌价值低:
  - 全好形`平和`/`断幺`手牌: 门前清提升打点.
  - 多愚形役牌 Nomi 手牌: 门前清保持防御力.
- `3077s66z`: 切 `3s` 保留役牌双碰进张, **速度**优于切 `7s` 固定赤宝牌 (役牌与赤宝牌**均为一番**).
- **绞杀**策略: 弃和或配弃时, 扣住**对手字牌**, 扣住**下家关键牌**, 阻碍对手作牌, 阻止后付和牌.

### 宝牌策略

- 宝牌: **早巡**保留宝牌及其靠张.
- **多张**宝牌:
  - 重视速度: 好形足够时可考虑门清立直, 一般情况下积极鸣牌速攻.
  - 多宝牌门清手牌最大化进张, 多宝牌副露手牌寻找手役加速和牌.
- 宝牌靠张 (特别是愚形靠张) 会降低和牌速度, 降低手牌防御力, 存在提升打点可能性.
- 孤张宝牌 (宝牌靠张) 切出时机:
  - 一般情况下, 手牌基本成形时, 切出孤张宝牌 (靠张).
  - 其余牌均对手牌速度/形状/打点有更大作用时, 切出孤张宝牌 (靠张).
  - 场况危险, 保留安全牌有更大作用时, 切出孤张宝牌 (靠张).
  - 一位/打点足够时更早切出孤张宝牌 (靠张).
  - 四位/打点不足时更晚切出孤张宝牌 (靠张).
- 处理浮牌与复合形时, 时刻考虑**宝牌进张**的影响.
  - 宝牌搭子: **不固定非宝牌雀头**, 保留宝牌搭子形成宝牌对子可能性 (1 番 -> 2 番).
  - 宝牌对子:
    重视宝牌双碰进张, 保留至少 2 组对子 (包括两面补强牌),
    保留宝牌对子形成宝牌刻子可能性 (2 番 -> 3 番).
  - 宝牌四连形: `3456` (`3` 为宝牌), 进 `3` 时可形成 `33` + `456`, 尽量保留四连形.
  - 宝牌连续形: `11234` (`4` 为宝牌), 用筋牌 `1` 作雀头, 进 `4` 时可切 `1` 极具迷惑性.
  - 宝牌复合形: `345566` (`3` 为宝牌), 保留 `6`, 进 `3` 时可形成 `45566` 一杯口复合形, 宝牌对子作**雀头**.
  - 重视宝牌复合形**延展性**:
    - 最大化宝牌进张期望, 充分利用**后续宝牌进张**: 优先固定**非宝牌复合形**, 保证宝牌复合形延展性.
    - 宝牌**靠边**时延展性最好.
...
- 打点不足时重视三色靠张, 打点足够时重视好形与速度.
- 愚形较多时, 三色副露可以加快速度, 三色确定愚形搭子价值高于两面搭子.
- 三色同顺作牌天平:
  - Slide 天平: 空切 Slide 成形面子, 最大化三色可能.
  - 两天平: `34m2345p34s` 保留四连形, 保留 `234`/`345` 三色可能.
...
- 不考虑三色同顺手役:
  ...
  - 三色部分以外已有**两组**成形**面子**, 三色同顺手役作成难度较高.

### 一气贯通手役

- 可以考虑一气贯通手役: 1 组面子 + 至少 6 牌.
- 打点不足时重视一气靠张, 打点足够时重视好形与速度.
- 愚形较多时, 一气副露可以加快速度, 一气确定愚形搭子价值高于两面搭子.
- 1~2 枚宝牌时, 平和 > 一气贯通.
- 多组断幺搭子时, 断幺九 > 一气贯通.
...
- 具有`立直`(**多好形**)/`断幺`/`三色`/`一气`等其他可能手役时, 不要强行染手.
- 中巡后染手手役未成形, 转牌效切牌, 不要强行染手.
- 可以考虑染手手役:
  - 混一色: 4 组搭子 + 至少 3 番.
  - 清一色: 至少 10 牌.
  - 3 组同色搭子 + 正常面子手最终形为愚形立直 Nomi: 强行染手追求满贯.
- 牌形较差的混一色手役要**重视防守**:
  - 高向听时不要轻易鸣牌.
  - 优先切中张给对手读牌压力, 保留**字牌**作为后续安全牌 (他人立直或听牌).
    (染手手役中的字牌在弃和后可用于防守).
- 牌形较好的混一色手役切牌时适当隐藏染手倾向 (e.g. 优先切边张), 降低字牌鸣牌难度.

### 全带幺手役

- 五组以上幺九搭子 + 对子时可以考虑全带幺手役, 优先切中张牌.
- 赤五规则中全带幺手役价值较低, 除特殊情况基本不考虑副露全带幺手役.
- 全带系手役自身价值不高 (作牌难度大 + 听牌多愚形 + 副露降番),
  全带三色/全带役牌/全带宝牌等满贯复合手役 (门清/副露) 才有作牌价值:
  - 手牌较差有至少 4 组幺九愚形搭子, 门清和率较低时, 中巡可以考虑全带副露加速和牌.
  - 混老头/清老头 = 对对和 + 全带幺.

### 七对子手役

- 四对子: **牌形与进张较差**时, 可考虑七对子手役.
- 五对子: 早巡七对子一向听, 可考虑转对对和手役.
- 六对子: 中晚巡七对子听牌, 禁止转对对和手役.
- 亲家作七对子优先考虑和牌率 (保留**幺九对子**), 子家作七对子可考虑打点 (**断幺七对子/门混七对子**).
- 早巡 3 对子 + 0/1 面子 + 多愚形, 可以考虑七对子手役:
...
- 曾打出过当前听牌 (胡牌形, 不考虑手役), 进入捨牌振听, 所有听牌都无法荣和, 只能自摸.
- 解除方法: 换听其他没有振听的牌.
...
- 振听先制立直, 可以压制对手, 是一个有效战术, 无需过度害怕振听立直.
- 和率优秀, 拒和损失点数少, 立直打点提升幅度大时,
  和牌的回报足以弥补振立的风险, 完全可以选择拒和振立.
  但一般情况下, 还是选择自摸和了比较好.
- 切过的牌容易鸣回失张, 无需过度担心振听搭子 (尤其是好形搭子).
...
- 无筋 19 `* 1.16` < 无筋 28 `* 1.10` < 无筋 37 `* 1.45` < 无筋 456.
- 筋19 < 两筋 456 < 筋 28 < 筋 37.
- 宣言筋 2378 铳率提升 5%, 宣言两筋 456 铳率提升 1~2%, 宣言筋 19 铳率约等于普通筋 19.
- 有相关牌 (+2/-2) 的情况下, 后筋普遍比前筋更安全, 排除愚形能降低筋牌铳率;
  对于纯粹的筋, 前筋铳率低于后筋的有筋 28、筋 37、两筋 46、半筋,
  前筋铳率高于后筋的有筋 19、两筋 5.
...
- 无壁牌: 零枚可见, 对应 ALL CHANCE 牌.
- 壁牌/薄壁牌外侧牌相对安全.
- 紧壁 NC (4+0) 外侧愚形少, 隔壁 NC (0+4) 外侧愚形多, e.g. 隔壁 NC (0+4) 外侧宣言筋 2378 铳率与无筋 2378 同一级别.
- 薄壁 OC 总体通常只能降低 2%~3% 铳率.
  紧薄壁 OC (3+0) 铳率大约为 AC (0+0) 的 50%,
  隔薄壁 OC (0+3) 铳率大约为 AC (0+0) 的 70%.
- 手牌 OC 铳率低于牌河 OC, 后者最后一张牌出现在立直家手牌概率更高.
- OC 铳率随巡目增长速率约等于无筋牌.

### 早巡外侧牌

- 早巡手切边张, 外侧牌安全: 早巡切牌可能为浮牌.
- 手切次数越多, 早外牌越安全.
- 紧邻早外牌铳率低于隔一早外牌.
- 早巡捨牌外侧牌铳率下降, 早巡捨牌筋外侧牌铳率上升:
  e.g. 早巡打 3 `->` 7/8 铳率上升, 早巡打 5 `->` 1/9 铳率上升.
...
- 他人立直或多副露听高打点牌, 且自己和牌困难 (e.g. `23456` 向听/愚形搭子多且进张少) 时,
  除非情况特殊, 应完全弃和.
...
- 读速度: e.g. 断平系手切多种中张数牌, 染手系手切染手数牌, 全带系手切幺九字牌.
- 读打点: e.g. 损失牌效保持手牌安定度打点低, 全牌效推进有一定打点, 固定宝牌对子高打点, 复合多种手役满贯可能.
- 读形状.
- 读手役.
- 读关联牌: e.g. 晚于安全牌切出的立直宣言牌有可能为关联牌.
- 读牌山: e.g. 早巡数牌周边山存量高, OC/NC 外侧山存量高.
...
- 宝牌副露: 不应盲目对攻.
- 小牌副露: 不应直接弃和.
  e.g. 提前固定两面好形并保留安牌 (损失一定牌效以保持手牌安定度),
  小牌速攻概率上升.
- 早巡好形副露: 存在高打点可能.
- 幺九副露存在役牌/染手/三色/一气/全带/对对可能.
- 早巡固定愚形存在作三色/一气/全带可能.
- 碰幺九宝牌存在役牌后付/对对和可能.
- 三副露 + 拆搭子: 单骑听牌可能.
...
- 非最后副露捨牌跨筋.
- 手切安全牌 (重要牌) 前捨牌跨筋: 即使放铳, 铳点可能较小, 小牌更倾向于反牌效留安.
- 两坎副露同色牌.
- 碰牌后捨牌跨筋可能通过的情况:
  - 根据 `两对子复合形 + 其他两面`, `两对子复合形 + 其他对子` (三对子形) 两种情况推理手牌.
  - 完全一向听碰牌后捨牌跨筋较为安全:
    e.g. `678m334s34p788p`, 先切 `7p` 进入完全一向听, `88p` 碰牌后切 `4s`, 周边可能存在雀头.
  - 愚形副露后, 固定对子碰牌后捨牌跨筋较为安全:
    e.g. `24678m34488p688s`, `24m` 吃牌后应切 `4p`, 而不是 `6s`,
    故 `24m` 吃牌后切 `6s`, `88s` 碰牌后切 `4p`, 周边可能存在雀头,
    牌形可能为 `24678m33478p688s`.
- 吃牌后捨牌跨筋可能通过的情况:
  吃牌前其他家切出该牌, 副露家没有碰听, 说明不存在捨牌周边复合形 (即 XXY 形),
  因此吃牌后捨牌跨筋可能安全, 也可能捨牌周边存在愚形听牌
  (+3 牌可能点坎张, +2 牌可能点双碰, +1 牌可能点单骑).
- 拆两面好形搭子, 弱于该形状的搭子跨筋比较安全:
  e.g. 二度受搭子弱于两面好形搭子, 二度受比较安全.

#### 副露危险牌

- 手切安全牌 (重要牌) 后, 吃牌时捨牌跨筋:
  - 该捨牌比安全牌 (重要牌) 更晚切出, 大概率为关联牌.
  - 两面吃牌时, 该捨牌跨筋以外的筋组较为安全, 不会放铳好形 (可能放铳愚形).
  - 例外情况 (捨牌不是关联牌):
    - 愚形吃牌时, 该捨牌也有可能为改良牌, 而非关联牌.
    - 暗刻作雀头.
    - 食延换听.
    - 复合形.
- 最后副露捨牌跨筋:
...
- 空切可能 (Slide):
  ...
  - 早巡手切 `3m`, 中巡手切 `1m`:
    若为 Slide 空切, 则万子下段存在完整面子, 较为安全,
    同时排除 `47m` 两面 (`23456m` 存在振听).
- 早巡多次摸切: 超好形或进张极差.
- 早巡切牌多为**断幺牌**, 后切**生张字牌**:
...
- `高价值浮牌早切`/`低价值浮牌晚切`: 留有特定形状的可能性变高, 特定牌的危险度上升.
  - e.g. 早切 `5`, 里筋 `14`/`69` 危险度上升:
    早切 `5`, 跨筋 `36`/`47` 危险度下降, 同时否定 `25`/`58` 两面,
    剩下 `14`/`69` 两组筋危险度上升, e.g. `235` / `578`.
  - e.g. 晚切 `5`, 里筋/跨筋危险度基本不变, 存在多种情况: `235`, `556`, 靠张一向听.
  - e.g. 早切 `8`, 里筋/跨筋危险度基本不变, 存在多种情况:孤张 `8`, `568`, `788`, `899`.
  - e.g. 晚切 `8`, 跨筋 `69` 危险度上升: `788`, `899`.
  - e.g. 早切 `3` 与 `8` (间四间):
    跨筋 `14` / `25` / `69` 危险度下降, 同时否定 `36` / `58` 两面,
    剩下 `47` 一组筋危险度上升.
    但这仅仅意味着间四间 `47` 比普通无筋 `47` 更危险,
    也无法真正地否定 `14` / `25` / `69` 两面.
- 手切**好形搭子** (e.g. `56` 两面搭):
  - 好形搭子超载, 手牌**速度较快**.
  - 好形听牌可能, 可用**筋壁理论**防守.
  - 消除二度受进张可能: `3344m`/`3467m` 手切 `34m`, 二度受进张 `258m` 危险度上升.
  - 受`宝牌`/`手役`(三色/一气/全带/七对)/`枚数`影响, 存在**更弱好形搭子**/**愚形搭子**听牌可能.
  - 手切两面搭子, 且牌河存在变则手可能: 字牌铳率倍增.
- 手切**愚形搭子** (e.g. `75` 坎张搭): 愚形搭子超载, 手牌**速度较快**.
- 顺切愚形搭子并立直: 宣言牌跨筋铳率上升 (关联牌).
- 逆切好形搭子并立直: 宣言牌跨筋铳率上升 (二度受).
- 手切对子:
  - 对子跨筋铳率下降.
...
- 立直宣言牌一般为以下三种:
  ...
  - 改良浮牌.
  - 改良靠张: e.g. 两面对补强牌.
- 好形放铳率: 1 / (18 - N), `123789` + 1, `456` + 2.
- 愚形放铳率:
  - 立直前手切 2 枚以上中张数牌, 数牌周边存在愚形可能大幅度降低:
    - e.g. `2mX7mX4m` 万字数牌可能全好形.
    - e.g. `9mXXXX4m` 听边 `7m` 概率大幅下降.
    - e.g. `9m8mXX4m` 听坎 `7m` 概率大幅下降.
  - 立直前手切 1 枚中张数牌, 未切一张远端边张牌, 存在前筋引挂可能:
    - e.g. `1mxx4mxx`, 未切过 `89m`, 存在听边 `7m` 可能.
- 讨论宣言牌对铳率的影响时, 实际是在讨论宣言牌是**关联牌**的可能性:
  - 关联牌: 听牌为宣言牌周边.
  - 入目: 手牌与宣言牌有关, 宣言牌周边进张后切出宣言牌立直, 听牌与宣言牌无关.
  - 靠张: 手牌为靠张一向听, 另外一个数牌靠张后切出宣言牌立直, 听牌与宣言牌无关.
- 边张宣言牌 (牌效低) 为关联牌概率高, 同色牌铳率上升.
- 立直宣言牌周边的前筋铳率上升: e.g. 切 4 立直, 筋 2 铳率上升.
- 手切生张役牌立直: **双碰/单骑字牌**听牌可能.
- 手切熟张字牌 (安全牌) 立直: **好形**听牌可能.
- 手切壁外数牌立直: **壁外愚形**听牌可能.
...
- 亲家早巡立直, 存在愚形听牌与低打点的可能:
  - 亲家牌河: 立直前手切对子/搭子/宝牌靠张, 亲家手牌速度快, 打点高, 优先考虑弃和.
  - 手牌价值: 立直 `Nomi` / 愚形听牌, 优先考虑弃和.
...
- 役牌:
  ...
  - 役牌早打: 无役牌或役牌暗刻的可能性高.
  - 役牌晚打: 役牌后付的可能性高 (期待单役牌变成双役牌, 防止后付振听).
- 断幺九:
  - 手切幺九对子.
  - 牌河均为幺九牌及其靠张.
  - 早巡切边张搭子.
  - 副露时, 手切安全牌 (重要牌) 后, 再手切幺九牌, 则可能不是断幺九.
  - 副露时, 保留断幺不确搭子 (e.g. `78m`), 拆掉断幺确搭子 (e.g. `22s`), 则可能不是断幺九.
- 平和:
  - 早巡手切 `字牌 -> 19 -> 28 -> 34567`.
  - 幺九数牌及其靠张切牌顺序晚于断幺九手役.
  - 中巡手切边张搭子.
...
- 南风局和牌局消化, 追求和率, 不追求打点.
  - 速攻结束牌局.
  - 速攻抢庄.
- 电报完场: 处于大 TOP ALL LAST 状态, 必要时可以帮助副露家和牌, 结束牌局.
- 电报过庄: 亲家 (特别是二位亲家) 立直, 必要时可以点报子家小牌, 过庄推动牌局.
...
- 当牌形较好时, 可以损失一定牌效去追求打点 (平和/断幺/三色/染手).
...
- 基本符数: 20 符.
- 刻子符数:
  2 (明刻) -> 4 (暗刻) -> 8 (明杠) -> 16 (暗杠),
  幺九刻子**翻倍**.
- 雀头符数: 役牌 2 符, Double 役牌 4 符.
- 听牌符数: 边张/坎张/单骑听 2 符, 自摸 2 符, 门清荣和 10 符.
- 特殊情况: 七对子 25 符, 平和自摸 20 符, 平和荣和 30 符.
...
- `LuckyJ` [AI](https://x.com/LuckyJ1443836).
